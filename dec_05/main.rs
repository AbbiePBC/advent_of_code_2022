...
fn do_instruction(instruction: &str) {
    let (num_stacks, start_point, end_point) = parse_instruction(instruction);
...
fn parse_instruction(instruction: &str) -> (i32, i32, i32) {
    ...
    return (
        inst[POSITION_NUM].parse::<i32>().unwrap(),
        inst[POSITION_START].parse::<i32>().unwrap(),
        inst[POSITION_END].parse::<i32>().unwrap(),
    );
}
